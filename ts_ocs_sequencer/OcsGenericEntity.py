...
class OcsGenericEntity(object):
    ...
    def __init__(self, system='', entity='', simulate=True):
        ...
        # abort data structure (cf. data = camera_command_abortC())
        self.__abortC = self._get_sal_cmdC('abort')

        # disable data structure (cf. data = camera_command_disableC())
        self.__disableC = self._get_sal_cmdC('disable')

        # enable data structure (cf. data = camera_command_enableC())
        self.__enableC = self._get_sal_cmdC('enable')

        # entercontrol data structure (cf. data = camera_command_enterControlC())
        self.__entercontrolC = self._get_sal_cmdC('enterControl')

        # exitcontrol data structure (cf. data = camera_command_exitControlC())
        self.__exitcontrolC = self._get_sal_cmdC('exitControl')

        # setvalue data structure (cf. data = camera_command_setValueC())
        # self.__setvalueC = self._get_sal_cmdC('setValue')

        # standby data structure (cf. data = camera_command_standbyC())
        self.__standbyC = self._get_sal_cmdC('standby')

        # start data structure (cf. data = camera_command_startC())
        self.__startC = self._get_sal_cmdC('start')

        # stop data structure (cf. data = camera_command_stopC())
        self.__stopC = self._get_sal_cmdC('stop')

        # define generic command(s) help
        self.logger.debug("Defining help")
        self.generic_help = (
        ...
        self.logger.debug("Defined help ok")
    ...
    def setvalue(self, parameter='', value='', timeout=OCS_GENERIC_COMMAND_TIMEOUT):
        ...
        if not isinstance(value, str) or value == '':
            raise OcsGenericEntityException(OCS_GENERIC_ENTITY_ERROR_NOVAL, "value={0:s}".format(value))
        else:
            self._value = value
        if not isinstance(timeout, int) or timeout < 0:
            raise OcsGenericEntityException(OCS_GENERIC_ENTITY_ERROR_NOVAL, "timeout={0:s}".format(str(timeout)))
    ...
    def timeout(self, timeout=OCS_GENERIC_COMMAND_TIMEOUT):
        if not isinstance(timeout, int) or timeout < 0:
            raise OcsGenericEntityException(OCS_GENERIC_ENTITY_ERROR_NOVAL, "timeout={0:s}".format(timeout))
        else:
            self._timeout = timeout
